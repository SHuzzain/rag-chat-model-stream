...
export async function generateSuggestions(
  question: string,
  answer: string,
  scope?: { chatbotId?: string; organizationId?: string }
) {
  if (!question || !answer || answer.trim().length === 0) {
    return { suggestions: [] };
  }

  if (!scope?.chatbotId || !scope.organizationId) {
    return { suggestions: [] };
  }
  ...
  try {
    ...
    const contextText = searchResults
      .map((r, idx) => `[Chunk ${idx + 1}] (${r.sourceName})\n${r.content}`)
      .join("\n\n");
...
${contextText}`,
    ...
    console.error(
      "[Suggestions] Error generating grounded suggestions:",
      error
    );
    return { suggestions: [] };
  }
